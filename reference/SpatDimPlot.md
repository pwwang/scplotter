...
- object:
  ...
  A Seurat object (with spatial data) or a Giotto object. S3 methods
  dispatch to the appropriate spatial technology handler.
...
- group_by:
  ...
  or factor column in the object's metadata. For `SpatDimPlot()`, if
  `NULL` and the object has FOV data with features, defaults to
  `"molecules"`; otherwise defaults to `"Identity"` (the active cluster
  identities). For
  [`SpatFeaturePlot()`](https://pwwang.github.io/scplotter/reference/SpatFeaturePlot.md),
  `group_by` is ignored — use `SpatDimPlot` for categorical grouping.
  The special value `"molecules"` enables molecule-level visualization
  in FOV-based data.
...
- features:
  ...
  For `SpatDimPlot()`, features are treated as molecule names to plot at
  single-molecule resolution (FOV data only). Can include gene names,
  metadata column names, or dimension reduction components.
...
A `ggplot` object representing the spatial dimension plot with points
(and optionally image/shapes layers) colored by group membership. When
`group_by` or `features` produce multiple color mappings, the plot may
include a legend for each mapping via ggnewscale.
...
- `SpatDimPlot` and
  ...
  engine. The difference is conceptual: `SpatDimPlot` visualizes
  discrete groups, `SpatFeaturePlot` visualizes continuous values.

- For Giotto objects, when `features` is provided, the behavior
  automatically switches to molecule mode (`group_by = "molecules"`).

- The `layers` argument controls rendering order. Place `"image"` first
  when you want the tissue image as background, followed by `"shapes"`
  for cell boundaries, and `"points"` on top.

## Molecule-level visualization

In FOV-based technologies (Xenium, CosMx, MERSCOPE), setting
`group_by = "molecules"` and providing `features` as molecule names
switches to molecule-level rendering: individual transcript detections
are plotted as points colored by feature identity, limited to `nmols`
molecules per feature. This is analogous to
[`Seurat::ImageDimPlot()`](https://satijalab.org/seurat/reference/ImageDimPlot.html)
and is the default behavior when `features` is set on a Seurat FOV
object.

## Default grouping

When neither `group_by` nor `features` is set:

- For Seurat objects with FOV data, defaults to `group_by = "molecules"`
  if `features` is set.

- Otherwise, defaults to `group_by = "Identity"`, which maps to the
  active cluster identities in the Seurat object (i.e.,
  [`Seurat::Idents()`](https://satijalab.github.io/seurat-object/reference/Idents.html)).

- For Giotto objects, defaults to `group_by = "molecules"` if `features`
  is set.
