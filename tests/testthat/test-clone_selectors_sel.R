test_that("sel() returns selected elements without groups", {
    ...
    result <- sel(y < 20, data = df, in_form = "wide", return_ids = FALSE)
    expect_equal(result$x, c("A"))

    # works with a character expression as well
    result <- sel("y == 10", data = df, in_form = "wide", return_ids = FALSE)
    expect_equal(result$x, c("A"))
})
...
test_that("sel() returns selected elements with groups", {
    ...
    result <- sel(y < 30, data = df, groups = "group", in_form = "wide", return_ids = FALSE)
    expect_equal(result$x, c("B"))

    result <- sel(mean(y) == 35, data = df, groups = group, in_form = "wide", return_ids = FALSE)
    expect_equal(result$x, c("A", "B", "C", "D"))
})

test_that("sel() respects return_ids", {
    df <- data.frame(
        group = c("A", "A", "B", "B"),
        x = c("A", "B", "C", "D"),
        y = c(10, 20, 30, 40)
